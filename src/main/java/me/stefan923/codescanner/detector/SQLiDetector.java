...
public class SQLiDetector implements VulnerabilityDetector {
    ...
    public List<Vulnerability> detect(Node node, Map<String, Boolean> taintMap) {
        List<Vulnerability> sqlInjectionVulnerabilities = new ArrayList<>();
            ...
            if (methodName.equals("executeQuery") || methodName.equals("executeUpdate")) {
                for (Expression arg : mce.getArguments()) {
                    if (isTainted(arg, taintMap)) {
                        ...
                        sqlInjectionVulnerabilities.add(new Vulnerability("SQL Injection",
                                "Method call " + methodName + " uses tainted data.",
                                className, line));
                    }
                }
            }
